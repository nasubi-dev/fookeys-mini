...
import infoImg from "@/assets/img/ui/info.webp";
      ...
      <div class="relative flex justify-start">
        ...
        <div class="overText">
          <div class="flex justify-start items-center font-bold text-lg">
            <img v-if="characterName" :src="`/img/characters/${characterName}/normal.webp`" class="w-1/3 bottom-5 bg-clip-border" />
            <p>
              {{
                "🍖" +
                (p.player.sumFields.hungry -
                  (p.player.isSaleZeroHungry ? p.player.field.reduce((acc, card) => (card.isSale ? acc + card.hungry : acc), 0) : 0))
              }}
            </p>
